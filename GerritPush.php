 ...
 *
 * This is a PHP script which makes it easier to push changes into
 * a gerrit controlled repository
 *
 * PHP Version 5.4
 *
...
class GerritPush
{
     ...
     */
    const SCRIPT_NAME = "Techxplorer's Gerrit Push script";
    ...
    public function doTask()
    {
        ...
        if (!$git->isClean()) {
            \cli\err("%rERROR: %wUntracked changes present, refusing to push\n");
            die(1);
        }

        // push the changes
        if (!$git->pushChangesToGerrit()) {
            die(1);
        }
    }
}
